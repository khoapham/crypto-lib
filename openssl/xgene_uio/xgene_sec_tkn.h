...
struct sec_tkn_ctx {
	...
	u64 result_tkn_hwptr;	/* PHY address of result_tkn_ptr right shifted by 4 */
 ...
 *	 IV3.
 */
u32 sec_tkn_set_crypt(u32 * t, u32 pl, u64 sa, u32 * ctrl, u8 ca, u8 cm,
		      u32 * nonce, u8 * iv, int enc);
...
unsigned short sec_tkn_set_cbc_authenc(unsigned int *t, unsigned int pl,
				       unsigned long long sa,
				       unsigned int *ctrl,
				       unsigned char ca,
				       unsigned int pdl,
				       unsigned char icvsize, int enc);
inline void sec_tkn_update_cbc_authenc(unsigned int *t, unsigned int pl,
				       unsigned int *ctrl,
				       unsigned char ca,
				       unsigned int pdl,
				       unsigned char icvsize, int enc);

static inline int xgene_sec_is_tkn_free(struct sec_tkn_ctx *tkn)
{
	return tkn->context == NULL;
}

static inline void xgene_sec_tkn_free(struct sec_tkn_ctx *tkn)
{
	tkn->context = NULL;
}
